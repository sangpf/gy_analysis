...
public class UserLoginHandlerInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
            throws Exception {
        System.out.println("=====================>> 进入 gy_analysis 拦截器 ...begin ...");
        // 获取请求的类型
        String type = request.getHeader("X-Requested-With");// XMLHttpRequest

        // 获取cookie的值 , cookie的值就是 token
        String token = CookieUtils.getCookieValue(request, COOKIE_NAME);
        log.info("=====================>> gy_analysis 服务拦截器, 从cookie中获取 COOKIE_NAME = USER_TOKEN 值为 : "+token);
        System.out.println("=====================>> gy_analysis 服务拦截器, 从cookie中获取 COOKIE_NAME = USER_TOKEN 值为 : "+token);

        User user = null;
        if (StrUtils.isNotEmpty(token)){
            // 根据token 获取user
            log.info("=============>> gy_analysis 服务 拦截器 ,token 不为空 :"+token+", 请求sso, 获取user信息 begin .. " );
            System.out.println("=============>> gy_analysis 服务 拦截器 ,token 不为空 :"+token+", 请求sso, 获取user信息 begin .. " );
            user = this.tokenService.getUserByToken(token);
            log.info("======================>> gy_analysis 服务 拦截器 ,请求sso服务, 获取 user :"+user);
            System.out.println("======================>> gy_analysis 服务 拦截器 ,请求sso服务, 获取 user :"+user);
        }
        ...
        if (StrUtils.isEmpty(token) || null == user) {
            ...
            if (StringUtils.equals("XMLHttpRequest", type)) {
                log.info("=======================>> gy_analysis 服务 拦截器 , 此请求为 ajax请求 ");
                ...
                log.info("=======================>> gy_analysis 服务 拦截器 , 此请求为 一般请求, 下面还是重定向 ");
